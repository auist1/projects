...
SELECT order_id, item_id, product_id,quantity,list_price,discount, quantity*list_price*(1-discount) AS totalrevenue
FROM Order_items

SELECT 
	YEAR(order_date) AS Years, SUM(TotalRevenue) AS TotalRev
FROM 
	Orders o
LEFT JOIN 
...
SELECT
	...
	SUM(totalrevenue) AS TotalRev,
	SUM(oi.quantity) AS TotalQuantity,
	ROUND(SUM(totalrevenue)/SUM(oi.quantity),0) AS UnitPrice
FROM 
	Orders o
JOIN 
	orderitems oi ON o.order_id=oi.order_id
...
WITH TR AS (
	SELECT 
		s.store_name, 
		ROUND(SUM(totalrevenue),0) AS TotalRev
	FROM Stores s
...
TR2 AS (
	SELECT ROUND(SUM(totalrevenue),0) AS TTR
	FROM Stores s
	LEFT JOIN Orders o ON s.store_id=o.store_id
	LEFT JOIN orderitems oi ON oi.order_id=o.order_id
)
...
TTC AS (
	SELECT 
		ROUND(SUM(totalrevenue),0) AS TTotalRev
	FROM orderitems oi
)
SELECT 
	TC.category_name,
...
SELECT 
	...
	ROUND(SUM(totalrevenue) / COUNT (DISTINCT o.order_id),0) AS BasketSize
FROM
	orderitems oi 
LEFT JOIN 
	Orders o 
...
SELECT TOP 10
	...
	MIN(totalrevenue) AS Minimum,
	AVG(totalrevenue) AS Average,
	MAX(totalrevenue) AS Maximum
FROM
	Products p
LEFT JOIN 
	orderitems oi 
...
SELECT 
    p.product_name,
    AVG(CASE WHEN YEAR(o.order_date) = 2016 THEN totalrevenue ELSE NULL END) AS 'Revenue_2016',
    AVG(CASE WHEN YEAR(o.order_date) = 2017 THEN totalrevenue ELSE NULL END) AS 'Revenue_2017',
    AVG(CASE WHEN YEAR(o.order_date) = 2018 THEN totalrevenue ELSE NULL END) AS 'Revenue_2018'
FROM
    Products p
LEFT JOIN 
    orderitems oi
